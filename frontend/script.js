...
function renderAll() {
    ...
    if (gameState.state === 'LOBBY') {
        ...
        status = `Masa Doluyor (${count}/4)...`;
        document.getElementById('btn-start').style.display = (count === 4) ? 'block' : 'none';
    } else {
        document.getElementById('btn-start').style.display = 'none';
        if (gameState.state === 'BIDDING') status = `İhale: ${gameState.players[gameState.bidding_turn]}`;
        else if (gameState.state === 'PLAYING') status = `Sıra: ${gameState.players[gameState.current_turn]}`;
    }
    ...
    for (const [ui, gp] of Object.entries(rel)) {
        ...
        if (area) {
            area.querySelector('.player-name').innerText = (gp === myPos ? "Sen" : name);
